...
const navigation = useNavigate()
    ...
    const allUsers = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:5001/api/admin/allusers",
          {
            headers: {
  ...
  const blockUser = async (id, isBlocked) => {
    ...
    const response = await axios.put(
      `http://localhost:5001/api/admin/blockuser/${id}`,
      { isBlocked: !isBlocked }, 
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const CommunitiesTab = () => {
  ...
  useEffect(() => {
    const viewCommunities = async () => {
      try {
        const token = localStorage.getItem('adminAuthToken');
        const response = await axios.get('http://localhost:5001/api/admin/viewcommunities', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
...
const ReportedPostsTab = () => {
  ...
  useEffect(() => {
    const viewReportedPost = async () => {
      try {
        const response = await axios.get('http://localhost:5001/api/admin/reportedpost');
        const { status, message, data } = response.data;
