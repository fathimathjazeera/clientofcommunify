...
function Community() {
  ...
  const viewSpecificCommunity = async () => {
    try {
      const response = await axios.get(`http://localhost:5001/api/users/singlecommunity/${communityname}`);
  ...
  const viewCommunityPost = async () => {
    try {
      const response = await axios.get(`http://localhost:5001/api/users/communitypost/${communityname}`);
      const { status, message, data } = response.data;
      if (status === 'success') {
  ...
  const joinCommunity = async (id) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5001/api/users/joincommunity/${id}`, {}, {
        headers: {
